...
#'  @param approxClrSTRT # added by Greg Booth The approximate position of the start of clearing wave.  
#'  Suggest using 2000 [bp/ min] * time [min], for mammalian data. Only used when emissionDistAssumption = 2waves
...
polymeraseWaveBW <- function(reads1_plus, reads1_minus, reads2_plus, reads2_minus, genes, approxDist, approxClrSTRT, size=50, 
  ...
  for(i in 1:NROW(genes)) {
    ...
    uTrans<- as.integer(ceiling((upstreamDist-250)/size))
    ...
    if(emissionDistAssumption == "norm") {
      ...
      parPsi  <- RgammaMLE(gene[c((uTrans+1):iTrans)])
      parBas  <- RgammaMLE(gene[c((iTrans+1):NROW(gene))])
      ePrVars <- data.frame(c(parInt$shape, parInt$scale, -1), ## this was causing issues becuase it was looking for output from using normal distribution (c(parInt$mean, sqrt(parInt$var), -1)
                            c(parPsi$shape, parPsi$scale, -1), 
                            c(parBas$shape, parBas$scale, -1))
    }
    else {
    ...
    ansVitervi <- ans[[3]][[1]]
    cat("printing ansVitervi:\n") # GB mod
    print(ansVitervi) # GB mod
