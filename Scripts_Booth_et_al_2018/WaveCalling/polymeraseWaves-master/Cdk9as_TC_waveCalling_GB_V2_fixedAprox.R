...
## Note Charles made subtle changes (05-09-17) to attempt to simultaneously call advancing waves and clearing waves in data. (I will try to comment on the Mods below)
...
fig_dir = "/Volumes/SEAGATE_EXP/Fisher_collaboration/analysis/figures/cdk9asTC_waves/10-05-17/"
dir.create(fig_dir)
...
# Note: I intentionally subtracted 500 from the TSS.
SPAPB1E7.07 <- data.frame(chr = "chr01", start = 3302325, end = 3310369, 
                          str = "+", SYMBOL = "SPAPB1E7.07", ID = "testGene",
                          stringsAsFactors = F)

test_1min_pw2 = polymeraseWaveBW(reads1_plus = oneMinBW.pl, reads1_minus = oneMinBW.mn, 
                                reads2_plus = zeroMinBW.pl, reads2_minus = zeroMinBW.mn, 
                                finterWindowSize = 2000, TSmooth = NA, genes = SPAPB1E7.07, 
...
## loop entire script over desired window sizes
for (size in c(50, 100, 250, 500)){
...
idx = (GL[,3] - GL[,2] >= 4000)
# GB edit 10-03-17: Try running with a fixed approx distance, rather than varied by time point. Want to make wave calls as unbiased as possible.
## otherwise comment out
dir.create(paste(fig_dir, "/fixedApproxDist_2kb/", sep = ""))
dir.create(paste(fig_dir, "/fixedApproxDist_2kb/CombinedReps/", sep = ""))
dir.create(paste(fig_dir, "/fixedApproxDist_2kb/CombinedReps/", size, "bpWindowing/", sep = ""))
#dir.create(paste(fig_dir, "/CombinedReps/", sep = ""))
#dir.create(paste(fig_dir, "/CombinedReps/", size, "bpWindowing/", sep = ""))
## Modification:  Trying out emmisionDist = "2waves"
pw_30sec= polymeraseWaveBW(reads1_plus = halfMinBW.pl, reads1_minus = halfMinBW.mn, reads2_plus = zeroMinBW.pl, reads2_minus = zeroMinBW.mn, finterWindowSize = 1000,
                                 TSmooth = NA, genes = GL[idx,], approxDist = 2000, size = size, upstreamDist = upstreamDist, prefix = paste(fig_dir, "/fixedApproxDist_2kb/CombinedReps/", size, "bpWindowing/", "30secAdvWave", sep = ""), emissionDistAssumption = "gamma",
                                 returnVal = "alldata")
pw_1min = polymeraseWaveBW(reads1_plus = oneMinBW.pl, reads1_minus = oneMinBW.mn, reads2_plus = zeroMinBW.pl, reads2_minus = zeroMinBW.mn, finterWindowSize = 1000,
                                TSmooth = NA, genes = GL[idx,], approxDist = 2000, size = size, upstreamDist = upstreamDist, prefix = paste(fig_dir, "/fixedApproxDist_2kb/CombinedReps/", size, "bpWindowing/", "1minAdvWave", sep = ""), emissionDistAssumption = "gamma",
                                returnVal = "alldata")
pw_2.5min = polymeraseWaveBW(reads1_plus = two.5MinBW.pl, reads1_minus = two.5MinBW.mn, reads2_plus = zeroMinBW.pl, reads2_minus = zeroMinBW.mn, finterWindowSize = 1000,
                                  TSmooth = NA, genes = GL[idx,], approxDist = 2000, size = size, upstreamDist = upstreamDist, prefix = paste(fig_dir, "/fixedApproxDist_2kb/CombinedReps/", size, "bpWindowing/", "2.5minAdvWave", sep = ""), emissionDistAssumption = "gamma",
                                  returnVal = "alldata")
pw_5min = polymeraseWaveBW(reads1_plus = fiveMinBW.pl, reads1_minus = fiveMinBW.mn, reads2_plus = zeroMinBW.pl, reads2_minus = zeroMinBW.mn, finterWindowSize = 1000,
                                TSmooth = NA, genes = GL[idx,], approxDist = 2000, size = size, upstreamDist = upstreamDist, prefix = paste(fig_dir, "/fixedApproxDist_2kb/CombinedReps/", size, "bpWindowing/", "5minAdvWave", sep = ""), emissionDistAssumption = "gamma",
                                returnVal = "alldata")
# save the wave calling output above to an ".RData" file in the fig_dir directory
save(pw_30sec, pw_1min, pw_2.5min, pw_5min, file = paste(fig_dir, "/fixedApproxDist_2kb/CombinedReps/", size, "bpWindowing/", "cdk9_TC_WaveCalldat.RData", sep=""))
...
dir.create(paste(fig_dir, "/fixedApproxDist_2kb/Rep1/", sep = ""))
dir.create(paste(fig_dir, "/fixedApproxDist_2kb/Rep1/", size, "bpWindowing/", sep = ""))
...
pw_30sec_r1 = polymeraseWaveBW(reads1_plus = halfMinBWr1.pl, reads1_minus = halfMinBWr1.mn, reads2_plus = zeroMinBWr1.pl, reads2_minus = zeroMinBWr1.mn, finterWindowSize = 1000,
                           TSmooth = NA, genes = GL[idx,], approxDist = 2000, size = size, upstreamDist = upstreamDist, emissionDistAssumption = "gamma",
                           returnVal = "alldata")
...
pw_5min_r1 = polymeraseWaveBW(reads1_plus = fiveMinBWr1.pl, reads1_minus = fiveMinBWr1.mn, reads2_plus = zeroMinBWr1.pl, reads2_minus = zeroMinBWr1.mn, finterWindowSize = 1000,
                           ...
                           returnVal = "alldata")
# save the wave calling output above to an ".RData" file in the fig_dir directory
save(pw_30sec_r1, pw_1min_r1, pw_2.5min_r1, pw_5min_r1, file = paste(fig_dir, "/fixedApproxDist_2kb/Rep1/", size, "bpWindowing/", "cdk9_TCr1_WaveCalldat.RData", sep=""))
...
dir.create(paste(fig_dir, "/fixedApproxDist_2kb/Rep2/", sep = ""))
dir.create(paste(fig_dir, "/fixedApproxDist_2kb/Rep2/", size, "bpWindowing/", sep = ""))
...
pw_5min_r2 = polymeraseWaveBW(reads1_plus = fiveMinBWr2.pl, reads1_minus = fiveMinBWr2.mn, reads2_plus = zeroMinBWr2.pl, reads2_minus = zeroMinBWr2.mn, finterWindowSize = 1000,
                              ...
                              returnVal = "alldata")
# save the wave calling output above to an ".RData" file in the fig_dir directory
save(pw_30sec_r2, pw_1min_r2, pw_2.5min_r2, pw_5min_r2, file = paste(fig_dir, "/fixedApproxDist_2kb/Rep2/", size, "bpWindowing/", "cdk9_TCr2_WaveCalldat.RData", sep=""))
}
