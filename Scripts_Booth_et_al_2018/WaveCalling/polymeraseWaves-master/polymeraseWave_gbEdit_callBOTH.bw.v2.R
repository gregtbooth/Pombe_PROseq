...
## Run the model separately on each gene.
    ...
    #### Define the gene in terms of the windowed size.
        ...
        if(emissionDistAssumption == "2waves") { 
          ## Leave centered on 0 for the norm_exp/norm emission functions
          gene  <- gene +(-1)*(min(gene))+1 
          ## Must translate points if gamma distributed (gamma undefined <0).
        }
        if(emissionDistAssumption == "2waves_norm") { 
          ## Leave centered on 0 for the norm_exp/norm emission functions
          gene  <- gene +(-1)*(min(gene))+1 
          ## Must translate points if gamma distributed (gamma undefined <0).
        }
...
##      Make the initial guess +5kb --> approxDist.
        uTrans<- as.integer(ceiling((upstreamDist-50)/size))
...
#        }
    ...
    #### Set up initial paremeter estimates.
        ...
        if(emissionDistAssumption == "norm") {
            ...
            parPsi  <- RgammaMLE(gene[c((uTrans+1):iTrans)])
            parBas  <- RgammaMLE(gene[c((iTrans+1):NROW(gene))])
            ePrVars <- data.frame(c(parPsi$shape, parPsi$scale, -1), 
                                c(parPsi$shape, parPsi$scale, -1), 
                                c(parBas$shape, parBas$scale, -1))
        }
        else if(emissionDistAssumption == "2waves_norm") {
          ePrDist <- c("gamma", "norm", "norm")
          parPsi  <- RgammaMLE(gene[c((uTrans+1):iTrans)])
          parBas  <- RgammaMLE(gene[c((iTrans+1):NROW(gene))])
          ePrVars <- data.frame(c(parPsi$shape, parPsi$scale, -1),
...
#           c(meanBas, stdeBas, -1)))
    ...
    ## Now run the HMM.
        ...
        ansVitervi <- ans[[3]][[1]]
        cat("here are the states represented by ansVitervi:\n")
        print(ansVitervi)
        ...
        if((DTs >= 1) & (DTe > 1) & (DTs < DTe) & 
            ...
            minMax <- max(MovMax[c(min(which(ansVitervi == 1)) ## gbEdit: changed from min to max (want to see if maximum window in center is less than median in advancing wave)
                    :max(which(ansVitervi == 1)))])
            minWindLTMed[i] <- (medDns > minMax) 
            ## Greg Booth edit 5/22/17: swapped the direction of inequality since 1st state is now within advancing wave (not upstream)
            ## True if min(wave) < med(wave.upstream) 
            avgDns <- median(MovMean[c(max((which(ansVitervi == 1))+
                    round(MovMeanSpd/size)):NROW(MovMean))])
            minAvg <- max(MovMean[c(min(which(ansVitervi == 1)) ## gbEdit: changed from min to max (want to see if maximum window in center is less than median in advancing wave)
                    :max(which(ansVitervi == 1)))])
