...
wigset = rbind(c("ePROseq_Pombe_CDK9as_0min_DMSO_COMBINED_pombe_plus.bw", "ePROseq_Pombe_CDK9as_0min_DMSO_COMBINED_pombe_minus.bw", "Cdk9as_0min_combined", NFs[26,'Spikereads']/100000),
...
library(ggplot2)
makeboxplots.TI_TEI <- function(postCPS_data, sampleList, sampleLabs = c("sample1", "sample2"), filename = "PI_quantile_PRdensity_boxplot.pdf"){
  ...
  for(i in 1:length(sampleList)){
    ...
    reformatRes = rbind(reformatRes, TIdat, TEIdat)
  }
  p <- ggplot(data = reformatRes, aes(x = factor(sample, labels = sampleLabs), y = log10(ratio), 
...
Zero_v_20_SampleList = c("Cdk9as_0min_combined", "Cdk9as_20min_combined")
Zero_v_10_SampleList = c("Cdk9as_0min_combined", "Cdk9as_10min_combined")

sink(paste(fig_dir, "t_test_results.txt", sep = ""))
Zero_v_20_res = makeboxplots.TI_TEI(postCPS_data = postCPS_ratios, sampleList = Zero_v_20_SampleList, sampleLabs = c("0 min", "20min"), filename = "Cdk9as_0min_vs_20min_TerminationRatios_boxplot.pdf")
Zero_v_20_diffTest = diffTest_TI_TEI(postCPS_data = postCPS_ratios, sampleList = Zero_v_20_SampleList)
print("test results for 0min vs 20 min (TI and TEI):\n")
print(Zero_v_20_diffTest)
Zero_v_10_res = makeboxplots.TI_TEI(postCPS_data = postCPS_ratios, sampleList = Zero_v_10_SampleList, sampleLabs = c("0 min", "10min"), filename = "Cdk9as_0min_vs_10min_TerminationRatios_boxplot.pdf")
Zero_v_10_diffTest = diffTest_TI_TEI(postCPS_data = postCPS_ratios, sampleList = Zero_v_10_SampleList)
print("test results for 0min vs 10 min (TI and TEI):\n")
print(Zero_v_10_diffTest)
sink()
#####################################################################################
...
GL_TEIsort_0vs20min = orderGenesbyTI(genes = bed, CPS_TI_Table = postCPS_ratios, samplename = "Cdk9as_0min_combined", TIorTEI = "TEI")
# prepare fold change heat data 
Zero_v_20_heat_CPS_TEIsort = compareHeats(GL_TEIsort_0vs20min[,c(1,3,2,4,5,6)], BWset1 = wigset[1,], BWset2 = wigset[8,], maxLength = 1000, step = 10, BWpath = bwpath)
Zero_v_10_heat_CPS_TEIsort = compareHeats(GL_TEIsort_0vs20min[,c(1,3,2,4,5,6)], BWset1 = wigset[1,], BWset2 = wigset[7,], maxLength = 1000, step = 10, BWpath = bwpath)
# plot the fold change heat maps
Plot3Heat(Zero_v_20_heat_CPS_TEIsort, filename = "0min_vs_20min_FCheat_aroundCPS_TEISort.pdf", distToTSS = 1000)# RAWbreaks = seq(0, 3, length.out = 100), FCbreaks = seq(-2,2, length.out = 100))
Plot3Heat(Zero_v_10_heat_CPS_TEIsort, filename = "0min_vs_10min_FCheat_aroundCPS_TEISort.pdf", distToTSS = 1000)# RAWbreaks = seq(0, 3, length.out = 100), FCbreaks = seq(-2,2, length.out = 100))
